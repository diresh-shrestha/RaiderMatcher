 ...
 */
      ...
      .findOne({ course: course }, function(err, foundCourse) {
        if (!foundCourse) {
          ...
          if (studentToRemoveTag) {
            try {
              studentsReadOne(studentToRemoveTag)
                .then((foundStudent) => {
              ...
              console.log("Error removing student from a course's students in coursesUpdateOne");
              console.log(err);
              return -1;
            }
          ...
          if (studentToAddTag) {
            ...
            studentsReadOne(studentToAddTag)
              .then((foundStudent) => {
                if (foundStudent) {
  ...
  const coursesDeleteOne = function(course) {
    if (course) {
      Course
        .deleteOne({ course: course })
        .exec((err, deletedCourse) => {
          if (err) {
            console.log(err);
          } else if (!deletedCourse) {
            // This doesn't work because
            console.log("Error - Course doesn't exist so can't delete in coursesDeleteOne");
          } else {
